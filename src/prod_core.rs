                ...
                l if l == limit_extra as usize => {
                    ...
                    (
                        ...
                        Some(
                            SeekLink::new(
                                Seek {
                                    anchor: Anchor::After(OrderDirection::Ascending, aname, aid),
            ...
            if projects.len() == limit_extra as usize {
                ...
                (
                    Some(
                        SeekLink::new(
                            Seek {
                                anchor: Anchor::Before(OrderDirection::Ascending, bname, bid),
                                sort_by: SortBy::ProjectName
                            }
                        )
                    ),
...
mod test {
    ...
    async fn get_projects_start_ok(pool: Pool) {
        ...
        let next_page = Some(
            SeekLink::new(
                Seek {
                    anchor: Anchor::After(OrderDirection::Ascending, "e".into(), 5),
                    sort_by: SortBy::ProjectName
                }
            )
        );
    ...
    async fn get_projects_end_ok(pool: Pool) {
        ...
        let prev_page = Some(
            SeekLink::new(
                Seek {
                    anchor: Anchor::Before(OrderDirection::Ascending, "f".into(), 6),
                    sort_by: SortBy::ProjectName
                }
            )
        );
